...
const Service = () => {
const[array,setarray]=useState({rang1:[],rang2:[],rang3:[],rang4:[],rang5:{},rang6:{}})
  ...
  const onsend=async(e)=>{
    e.preventDefault() 
       if(array.rang6.name&&array.rang6.tel){
        const docRef = await setDoc(doc(db, "services",array.rang6.id ), array); 
   ...
   router.replace("/") }else{ alert("لابد من وجود الاسم ورقم الجوال")  }
    ...
    return (
      ...
      <h5 className='text-center tt'>خدمة التأثيث</h5>

      <div className='w-100 mt-3 row mx-0 p-0 boxs  rang'>
      <div className='line  '></div>
       <div className='col-2 '>
       <div className={rang<1?'bill active-bill':'bill  dactive-bill'}></div>
       <p className={rang<1?'rang-bil active-billx ':'rang-bil  dactive-billx'}>نمط التأثيث</p>
       </div>
       <div className='col-2 '>
       <div className={rang<2?'bill active-bill ':'bill  dactive-bill'}></div>
       <p className={rang<2?'rang-bil active-billx ':'rang-bil  dactive-billx'}>نمط الستائر</p>
       </div>
       <div className='col-2 '>
       <div className={rang<3?'bill active-bill ':'bill  dactive-bill'}></div>
       <p className={rang<3?'rang-bil active-billx ':'rang-bil  dactive-billx'}>باقات إضافية</p>
       </div>
       <div className='col-2 '>
      ...
      <div className='container'>
       ...
       {rang==0? <Rang1 items={{array,setarray}}/>:""
       }
         {rang==1? <Rang2 items={{array,setarray}}/>:""
       }
       {rang==2? <Rang3 items={{array,setarray}}/>:""
       }
       {rang==3? <Rang4 items={{array,setarray}}/>:""
       }
       {rang==4? <Rang5 items={{array,setarray}}/>:""
       }
       {rang==5? <Rang6 items={{array,setarray}}/>:""
       }
      </div>
      <div className='direction row w-100  mt-2 mx-0 p-0 boxs'>
        ...
        {
          rang==5?            <img className="col-2 " src="next.svg" alt="" onClick={onsend}/>
          : 
